...
function Section({ title, children, loading }) {
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
      ...
      {loading ? (
        <div className="text-gray-500 text-sm text-center py-6">Yüklənir...</div>
      ) : children}
    </div>
  );
}
...
export default function BotStats() {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-white">📈 Bot Statistikası</h2>

      {/* Summary cards */}
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {[
          { icon: "👥", label: "Ümumi istifadəçi",    val: stats?.totalUsers   },
          { icon: "💰", label: "Ödənişli istifadəçi", val: stats?.paidUsers    },
          { icon: "📨", label: "Ümumi sorğu",         val: stats?.totalQueries },
          { icon: "📊", label: "Konversiya nisbəti",  val: `${convRate}%`      },
        ].map(({ icon, label, val }) => (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-800">
                {top10.map((u, i) => (
                  <tr key={u.telegram_id} className="hover:bg-gray-800/40 transition">
                    ...
                    <td className="py-2 px-3">
                      <span className={`px-2 py-0.5 rounded-full text-xs ${PLAN_COLORS[u.plan_name]} text-white`}>
                        {u.plan_name}
                      </span>
                    </td>
                    <td className="py-2 px-3 text-gray-300 font-mono">{u.queries_used}</td>
                    <td className="py-2 px-3 text-gray-400 text-xs">{u.last_active?.slice(0, 10) || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
