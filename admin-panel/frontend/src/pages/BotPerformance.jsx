...
function LatencyBadge({ label, value }) {
  const color = value == null ? "border-gray-700 text-gray-500"
    : value < 3000  ? "border-green-700/60 bg-green-950/40 text-green-300"
    : value < 6000  ? "border-yellow-700/60 bg-yellow-950/40 text-yellow-300"
    : "border-red-700/60 bg-red-950/40 text-red-300";
  return (
    <div className={`border rounded-xl p-3 sm:p-4 text-center transition-all duration-300 ease-smooth hover:-translate-y-0.5 ${color}`}>
      <div className="text-xl sm:text-2xl font-bold tabular-nums">{value != null ? `${(value / 1000).toFixed(2)}s` : "—"}</div>
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-white flex items-center gap-2">
        <span>🤖</span><span>Bot Performansı</span>
      </h2>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 sm:p-6 transition-all duration-300 hover:border-gray-700">
        <h3 className="text-sm sm:text-base font-semibold text-white mb-4 flex items-center gap-2">⏱ Cavab Vaxtı (Latency)</h3>
        <div className="grid grid-cols-3 gap-2 sm:gap-4 stagger-fast">
        ...
        <div className="mt-4 grid grid-cols-3 gap-2 sm:gap-4 text-center text-sm">
          <div className="bg-gray-800/70 rounded-xl p-3 transition-all duration-300 hover:bg-gray-800 hover:-translate-y-0.5">
            <div className="text-white font-bold tabular-nums">{qstats?.today?.total ?? "—"}</div>
            <div className="text-gray-500 text-[11px] mt-1">Bugün sorğu</div>
          </div>
          <div className="bg-gray-800/70 rounded-xl p-3 transition-all duration-300 hover:bg-gray-800 hover:-translate-y-0.5">
            <div className="text-white font-bold tabular-nums">{qstats?.week?.total ?? "—"}</div>
            <div className="text-gray-500 text-[11px] mt-1">Bu həftə</div>
          </div>
          <div className="bg-gray-800/70 rounded-xl p-3 transition-all duration-300 hover:bg-gray-800 hover:-translate-y-0.5">
            <div className={`font-bold tabular-nums ${perf?.error_rate > 5 ? "text-red-400" : "text-green-400"}`}>
              {perf?.error_rate != null ? `${perf.error_rate}%` : "—"}
            </div>
            <div className="text-gray-500 text-[11px] mt-1">Xəta nisbəti</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        {/* Hourly heatmap */}
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 sm:p-6 transition-all duration-300 hover:border-gray-700">
          <h3 className="text-sm sm:text-base font-semibold text-white mb-4 flex items-center gap-2">🕐 24 Saatlıq Aktivlik Xəritəsi</h3>
          {loading ? (
            <div className="grid grid-cols-12 gap-1">{[...Array(24)].map((_, i) => <div key={i} className="h-7 sm:h-8 skeleton rounded" />)}</div>
          ) : hourly.length === 0 ? (
              ...
              <div className="grid grid-cols-12 gap-1 mb-1">
                {Array.from({ length: 24 }, (_, h) => {
                  ...
                  return (
                    <div
                      key={h}
                      className={`${bg} rounded h-7 sm:h-8 flex items-center justify-center group relative cursor-default transition-all duration-500 hover:scale-110`}
                      style={{ animationDelay: `${h * 18}ms`, animation: "fade-in 0.5s cubic-bezier(0.22, 1, 0.36, 1) both" }}
                    >
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:block bg-gray-700 text-white text-[11px] px-2 py-1 rounded whitespace-nowrap z-10 shadow-lg">
                        {h}:00 — {count} sorğu
                      </div>
                    </div>
                  );
      ...
      {/* Popular topics */}
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 sm:p-6 transition-all duration-300 hover:border-gray-700">
        <h3 className="text-sm sm:text-base font-semibold text-white mb-5 flex items-center gap-2">📚 Ən Populyar Mövzular (Son 7 Gün)</h3>
        {loading ? (
          <div className="space-y-3">{[...Array(6)].map((_, i) => <div key={i} className="h-8 skeleton rounded" />)}</div>
        ) : topicsList.length === 0 ? (
          <p className="text-gray-500 text-sm text-center py-6">📭 Məlumat yoxdur</p>
        ) : (
          <div className="space-y-2.5 sm:space-y-3 stagger-fast">
            {topicsList.slice(0, 10).map((t, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-2 sm:gap-3">
                  <span className="text-gray-500 text-xs w-4 text-right tabular-nums shrink-0">{i + 1}</span>
                  <span className="text-gray-300 text-xs sm:text-sm w-24 sm:w-40 truncate shrink-0">{label}</span>
                  <div className="flex-1 h-2 bg-gray-800 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-blue-600 to-blue-400 rounded-full transition-all duration-700 ease-smooth" style={{ width: `${pct}%` }} />
                  </div>
                  <span className="text-gray-400 text-xs w-10 text-right tabular-nums shrink-0">{count}</span>
                </div>
              );
            })}
          </div>
